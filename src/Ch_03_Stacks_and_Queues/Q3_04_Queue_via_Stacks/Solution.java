...
import Ch_03_Stacks_and_Queues.common.MyStack;
...
class MyQueue<T> {
    ...
    MyStack<T> supportStack = new MyStack<>();
    ...
    public T remove() {
        supportStack = new MyStack<>();
        T val;
        for (int i =0 ;i< size; i++){
            val = mainStack.pop();
            supportStack.push(val);
        }
        T result = supportStack.pop();
        size--;
        for (int i =0 ;i< size; i++){
            val = supportStack.pop();
            mainStack.push(val);
        }
