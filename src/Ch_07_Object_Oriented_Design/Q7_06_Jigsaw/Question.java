...
public class Question {
    ...
    public static Edge[] createEdges(Piece[][] puzzle, int column, int row) {
        ...
        /* Get left edge */
        Edge left = column == 0 ? new Edge(Shape.FLAT, key + "h|e") : puzzle[row][column - 1].getEdgeByOrientation(Orientation.RIGHT)._createMatchingEdge();

        /* Get top edge */
        Edge top = row == 0 ? new Edge(Shape.FLAT, key + "v|e") : puzzle[row - 1][column].getEdgeByOrientation(Orientation.BOTTOM)._createMatchingEdge();
        ...
        for (int r = 0; r < solution.length; r++) {
            for (int c = 0; c < solution[r].length; c++) {
                ...
                if (c > 0) { /* match left */
                    ...
                    if (!left.getEdgeByOrientation(Orientation.RIGHT).fitsWith(piece.getEdgeByOrientation(Orientation.LEFT))) {
                        return false;
                    }
                }
                if (c < solution[r].length - 1) { /* match right */
                    Piece right = solution[r][c+1];
                    if (!right.getEdgeByOrientation(Orientation.LEFT).fitsWith(piece.getEdgeByOrientation(Orientation.RIGHT))) {
                        return false;
                    }
                }
                if (r > 0) { /* match top */
                    Piece top = solution[r-1][c];
                    if (!top.getEdgeByOrientation(Orientation.BOTTOM).fitsWith(piece.getEdgeByOrientation(Orientation.TOP))) {
                        return false;
                    }
                }
                if (r < solution.length - 1) { /* match bottom */
                    Piece bottom = solution[r+1][c];
                    if (!bottom.getEdgeByOrientation(Orientation.TOP).fitsWith(piece.getEdgeByOrientation(Orientation.BOTTOM))) {
                        return false;
                    }
                }
            }
